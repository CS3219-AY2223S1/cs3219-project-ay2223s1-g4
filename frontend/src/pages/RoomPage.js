...
function RoomPage() {
  ...
  useEffect(() => {
    getAccessTokenSilently().then((token) => {
      axios
        .get(`${URL_MATCHING_ROOM_SVC}/${roomId}`, {
        ...
        .then((res) => {
          ...
          let peerId =
            user.sub === res.data.userid1 ? res.data.userid1 : res.data.userid2;
          axios
            .get(`${URL_USER_SVC}/username/${peerId}`, {
              headers: { Authorization: `Bearer ${token}` },
            })
            .then((content) => {
  ...
  return (
    hasLoaded && (
      <Box>
        <Prompt
          message={
            "Are you sure you would like to unlock the solution for both you and your peer?"
          }
          isOpen={isPromptOpen}
          handleYes={closeRoom}
          handleNo={undoPrompt}
        ...
        <Button variant="contained" onClick={prompt}>
          Reveal Solution
        </Button>
      </Box>
    )
